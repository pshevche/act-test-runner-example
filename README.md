...
[![CI](https://github.com/pshevche/act-test-runner-example/actions/workflows/ci.yml/badge.svg)](https://github.com/pshevche/act-test-runner-example/actions/workflows/ci.yml)
...
- [Docker](https://docker.com) — required for e2e tests (act needs Docker containers)
...
Unit tests mock `@actions/core` and `@actions/github` to verify the action logic:
...
E2e tests use [act-test-runner](https://github.com/pshevche/act-test-runner) to run the action locally via `act` in Docker containers. A mock Express server stands in for the GitHub API so requests can be inspected:

```bash
npm run test:e2e
```

The CI workflow runs linter + unit tests first, and only if those pass, runs the e2e tests.
